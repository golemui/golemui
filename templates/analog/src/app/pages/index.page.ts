...
@Component({
  ...
  template: `
    <h1>GolemUI Analog Template</h1>
    <gui-form [config]="config" (formEvent)="onFormEvent($event)"></gui-form>
  `,
})
export default class HomePage {
  protected config: GuiFormInitConfig = {
    // Stable id: with server-side rendering the server and the browser must agree on the
    // form id, or hydration re-renders the form.
    formName: 'signup',
    formDef,
    formConfig: {
