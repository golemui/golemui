...
describe('generated-file markers', () => {
  // The in-repo implementations keep the manifest and the script name at these paths,
  // so their generated trees must not change when a config omits the new fields.
  it('name the in-repo manifest path and script when the config omits them', () => {
    const schema = buildWidgetsSchema(testConfig) as any;
    expect(schema.$comment).toBe(
      'GENERATED by libs/gui/schemas/tools/generate-schemas.ts from src/lib/widget-manifest.ts. ' +
        'DO NOT EDIT. Regenerate with `npm run generate:schemas`.',
    );
    expect(buildSchemasPackageIndex(testConfig)).toContain(
  ...
  it('name the manifest path and command an external implementation sets', () => {
    const externalConfig: ImplementationSchemaConfig = {
      ...testConfig,
    ...
    for (const schema of [
      buildWidgetsSchema(externalConfig),
      buildFormEnvelope(externalConfig),
      buildLayoutWidgetSchema(externalConfig),
    ]) {
      expect(schema['$comment']).toBe(
        'GENERATED by tools/generate-schemas.mjs from schemas.config.mjs. ' +
          'DO NOT EDIT. Regenerate with `npx @golemui/schemas generate`.',
      );
    }
    const source = buildSchemasPackageIndex(externalConfig);
    expect(source).toContain(
      '// GENERATED by tools/generate-schemas.mjs from schemas.config.mjs. DO NOT EDIT.',
    );
    expect(source).toContain('// Regenerate with `npx @golemui/schemas generate`.');
    expect(source).not.toContain('widget-manifest.ts');
  });
});
