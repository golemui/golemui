import { createShortcutType } from '@golemui/dx';
import { buildTypedValidator } from '@golemui/dx';
import { extractWidgetProps } from '@golemui/dx';
import { processAutoLabel } from '@golemui/dx';
import {
  type DateTimeCalendarDecorator,
  type DateTimeCalendarEntry,
  type GslDateTimeCalendarConfig,
} from './dateTimeCalendar.domain';

export const dateTimeCalendarShortcutType = createShortcutType<
  DateTimeCalendarEntry,
  DateTimeCalendarDecorator,
  GslDateTimeCalendarConfig
>({
  itemType: 'DATE_TIME_CALENDAR',
  kind: 'input',
  entryShape: 'bare',
  sensibleDefaults: {
    base: { suppressAutomaticLabels: false },
    fields: ['suppressAutomaticLabels'],
    apply: (def, config) => processAutoLabel(def, config),
  },
  mapToWidget: (def) => ({
    uid: def.uid ?? '',
    kind: 'input',
    type: 'dateTimeCalendar',
    path: def.path ?? '',
    ...(def.label != null ? { label: def.label } : {}),
    ...(def.disabled != null ? { disabled: def.disabled } : {}),
    ...(def.readonly != null ? { readonly: def.readonly } : {}),
    ...(def.validator != null
      ? { validator: buildTypedValidator(def.validator as any, 'string') }
      : {}),
    props: extractWidgetProps(def),
  }),
});

export const _gslDateTimeCalendars = dateTimeCalendarShortcutType.gsl;
export const _gslDateTimeCalendarByUid = dateTimeCalendarShortcutType.gslByUid;
