import { type OneOfProps, type Option, type OptionValue } from '../widget.props';
...
/**
 * Returns an array of normalized Options. The one normalization the choice widgets
 * (`select`, `radiogroup`) apply to their `options` prop: plain values become their own
 * label, objects are read through `labelField` / `valueField`.
 */
