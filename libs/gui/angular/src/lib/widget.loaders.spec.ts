...
// The awaited imports compile every widget module for the first time, which can take
// several seconds on a busy CI runner - so the timeout is raised.
describe('preloading the gui widget set outside a browser', () => {
  it('resolves every widget module in a plain node environment', { timeout: 30_000 }, async () => {
