...
describe('Timeinput schema validation', () => {
  ...
  describe('Valid configurations', () => {
    it('should validate a minimum valid timeinput definition', () => {
      ...
      const validTimeInput = formDef.form.children[0];
      const isValid = validate(validTimeInput);
      if (!isValid) {
        specValidationErrorsLogger(validate, validTimeInput);
      }
    ...
    it('should validate state-scoped properties', () => {
      ...
      const stateScopedTimeInput = formDef.form.children[0];
      const isValid = validate(stateScopedTimeInput);
      if (!isValid) {
        specValidationErrorsLogger(validate, stateScopedTimeInput);
      }
      expect(isValid).toBe(true);
    });
    ...
    it('should validate i18n localizable properties', () => {
      ...
      const i18nTimeInput = formDef.form.children[0];
      const isValid = validate(i18nTimeInput);
      if (!isValid) {
        specValidationErrorsLogger(validate, i18nTimeInput);
      }
      expect(isValid).toBe(true);
    });
  });
  ...
  describe('Invalid configurations', () => {
    it('should fail on invalid hourFormat value', () => {
      ...
      const invalidTimeInput = formDef.form.children[0];
      const isValid = validate(invalidTimeInput);
