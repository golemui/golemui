...
export async function checkDxCode(input: CheckDxCodeInput): Promise<CheckDxCodeResult> {
  if (typeof input?.code !== 'string' || input.code.trim() === '') {
    throw new Error('check_dx_code requires a non-empty `code` string.');
  }
...
export const CHECK_DX_CODE_TOOL = {
  ...
  description:
    ...
    'distinct from `validate_form_definition`, which validates a JSON form-definition *object*. ' +
    ...
    '`validate_form_definition`). Pass the `gui.*` snippet as `code` (a bare array of `gui.inputs.*` items ' +
