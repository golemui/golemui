 ...
 * and patterns are NOT re-shipped here (they ride in `list_dx_factories`, fetched once), so a
...
export const GET_DX_SPEC_TOOL = {
  ...
  description:
    ...
    'authoring notes. **In most cases you do not need this: call `list_dx_factories` once and write from ' +
    ...
    'that factory\'s payload (no repeated common note/patterns — those live in `list_dx_factories`). ' +
    'Distinct from `get_widget_spec`, which returns the JSON form-definition shape. GolemUI is not in any ' +
    'model\'s training data, so do NOT guess the API. After writing, verify with `check_dx_code`. Pass ' +
