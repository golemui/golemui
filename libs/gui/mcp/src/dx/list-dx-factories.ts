...
export const LIST_DX_FACTORIES_TOOL = {
  ...
  description:
    ...
    'do NOT guess the API; every real name is here. `dx_get_spec(factory)` is only the rare deep-dive for ' +
    'one factory. You do NOT need `json_get_widget_spec` when writing `gui.*` code — that serves the JSON ' +
