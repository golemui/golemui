...
export const DX_INSTRUCTIONS =
  ...
  'NOT guess the API. FIRST call `dx_list_factories` once: it is the COMPLETE reference — every factory ' +
  ...
  'forms. `dx_get_spec` is only a rare single-factory deep-dive — you usually will not need it, and you ' +
  'do NOT need `json_get_widget_spec` here (that is the JSON surface). Then ' +
  'ALWAYS finish by calling `dx_check_code` with the snippet — it type-checks against the real ' +
  ...
  '`dx_check_code` for `gui.*` *code* and `json_validate_form_definition` for a JSON *definition object* — ' +
