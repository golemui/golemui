...
export class DateTimeElement extends LitElement implements WithWidget {
  ...
  adapter = new InputWidgetAdapter<string, DateTimeInputProps>();
  ...
  override render() {
    ...
    // Icon
    const dateTimeIcon = addIcon('date-time', this.adapter.templateData);

    return html`
      <gui-date-time
        class=${classMap(dateTimeIcon.widgetClasses)}
        .uid=${this.widget.uid}
...
safeDefine('gui-date-time-input', DateTimeElement);
