...
- **JSON API** — a serializable `{ "form": [...] }` document.
  → Read [references/forms-json.md](references/forms-json.md).

**The request already states the API — do NOT ask, go ahead — when it names one, or when it
describes what the form IS in a way only one of them fits:**

- The form is **data that lives outside the code** — stored, served, versioned, generated by a
  tool, edited without recompiling, or shared by more than one app or stack → **JSON API**.
  (e.g. "a JSON definition", "served from our backend", "produced by an authoring tool", "a
  `.json` file per form")
- The form is **code that lives in the application** — built inside a component or module,
  next to the code that uses it, compiled, type-checked and refactored together with it →
  **TS API**. (e.g. "with `gui.*`", "an imperative form", "in TypeScript with autocomplete")

The examples are illustrations, not a list to match: judge by the criterion. Ask ONLY when the
request says nothing about either — "make me a signup form in React" says how it renders, not
how it is authored.
