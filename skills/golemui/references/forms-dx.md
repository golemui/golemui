...
- RENDER (React) - `import { gui } from '@golemui/gui-shared'; import { GuiForm } from '@golemui/gui-react'; import type { FormSubmitEvent } from '@golemui/core';`, then render `<GuiForm config={{ formDef: form }} formSubmit={(e: FormSubmitEvent) => { /* e.data is the form data */ }} />`. A `gui.displays.display(() => <h2>…</h2>)` returns React JSX. For SSR (Next.js) await `preloadFormWidgets({ widgetLoaders })` from `@golemui/core` before rendering on both server and client; `widgetLoaders` comes from `@golemui/gui-react`. In the App Router the preload belongs in the client component module that renders the form. Set an explicit `formName` and expect handlers to run in the browser only.
