...
const clientDir = join(here, '../../dist/apps/vue-ssr-harness/client');
const serverEntry = join(here, '../../dist/apps/vue-ssr-harness/server/entry-server.js');
...
server.listen(port, () => {
  ...
  console.log(`vue-ssr-harness (${mode}) on http://localhost:${port}`);
});
