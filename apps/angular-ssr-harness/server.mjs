...
// Unlike the other harnesses there is no placeholder replace: Angular's
// renderApplication takes the template as the document and returns the whole page.

const here = dirname(fileURLToPath(import.meta.url));
const clientDir = join(here, '../../dist/apps/angular-ssr-harness/client');
const serverEntry = join(here, '../../dist/apps/angular-ssr-harness/server/entry-server.js');
const isProduction = process.argv.includes('--production');
const port = Number(process.env.PORT ?? 3603);
  ...
  return createHttpServer((req, res) => {
    vite.middlewares(req, res, async () => {
      try {
        ...
        send(res, 200, await render(template));
      } catch (error) {
...
server.listen(port, () => {
  ...
  console.log(`angular-ssr-harness (${mode}) on http://localhost:${port}`);
});
