...
<IntegrationTabs>
  ...
  <Fragment slot="angular">
    ...
    **Key concepts (Angular)**
    ...
    - Provide `Angular.InputWidgetAdapter` in the component's `providers`, then `init()` it in `ngOnInit` and `destroy()` it in `ngOnDestroy`. The adapter's `templateData()` signal exposes `value`, `errors`, `touched`, plus your custom props.
