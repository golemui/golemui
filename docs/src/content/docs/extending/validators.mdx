...
In the JSON path, custom validators live on the **standalone `customValidators` prop** on the form component:
...
<IntegrationTabs>
  ...
  <Fragment slot="angular">
    ...
    // template: <gui-form [formDef]="formDef" [customValidators]="customValidators"></gui-form>
    ```
  </Fragment>
  <Fragment slot="lit">
    ```ts
    ...
    export class FormElement extends LitElement {
      formDef = formDef;
      customValidators = { allowedNames };
      // render: html`<gui-form .formDef=${this.formDef} .customValidators=${this.customValidators}></gui-form>`
    }
    ```
  </Fragment>
</IntegrationTabs>
...
```json
{
  "kind": "input", "type": "textinput", "path": "user.name",
  "validator": { "type": "custom", "allowedNames": ["John", "Jane"] }
}
```
...
The form engine resolves `allowedNames` against the registered map (`formConfig.customValidators` for Programmatic, the `customValidators` prop for JSON) at validation time.
