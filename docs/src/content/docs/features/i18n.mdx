...
The translator goes into a separate `localization` prop on your form component (it sits **alongside** `formConfig`, not inside it). Below uses the i18next adapter; the wiring is identical for any other adapter you build.
...
GolemUI re-renders every `Localizable` whenever the translator notifies its subscribers. So as long as your adapter calls `listener(lang)` after the language changes — which all three examples above do — calling your library's "change language" function (e.g. `i18next.changeLanguage('es')`) is enough to retranslate the whole form. No form-level reset, no remount.
