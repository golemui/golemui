...
```json
{
  "kind": "input", "type": "textinput", "path": "drivingTest",
...
```json
{
  "kind": "action", "type": "button", "uid": "save",
  "label": "Save",
  "disabled.busy": true,
...
```json
{
  "kind": "action", "type": "button", "uid": "login",
  "label": "Login",
...
gui.displays.alert('submit-hint', {
  ...
  level: 'warning',
  states: {
    'register:adult:canSubmit': { text: 'You can Register now', level: 'success' },
    'register:minor:canSubmit': { text: 'You can Register now', level: 'success' },
  },
});
```
