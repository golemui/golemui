...
A validator is just a `validator` object on an input widget, with the rules for that input's value shape plus an optional `messages` block. In the DX path the input's value type fixes the validator's shape, so you write only the rules (`validator: { minLength: 8, ... }`); in the JSON path the validator carries an explicit `type` discriminator (`validator: { type: 'string', minLength: 8, ... }`).
