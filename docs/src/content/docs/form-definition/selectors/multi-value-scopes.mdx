...
Only **Email** and **Username** carry both tags, so only those two pick up the hint. _Display name_ (one tag), _Short bio_ (one tag), and _Nickname_ (no tags) stay clean.
...
Three of the four fields carry at least one of the listed tags and pick up the hint. _Legacy key_ has no tags so the scope skips it.
...
Reads as: _for inputs tagged both `identity` and `required`, while the `readonly` state is active, disable them_.
