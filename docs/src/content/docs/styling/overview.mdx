...
Dark mode activates automatically based on the user's system preference. No JavaScript, no class toggles, no configuration — just import the stylesheet and it works. Every semantic token has both a light and a dark value; widgets switch between them seamlessly.

See [Theming](/styling/theming/) for built-in themes and how to build your own.
...
<IntegrationTabs>
  <Fragment slot="react">
    ...
    export function FormPage() {
      return (
        <GuiForm
          config={{ formDef, data: formData, formConfig, localization }}
        />
      );
    }
    ```
