...
Semantic tokens control how widgets actually look — backgrounds, text colors, borders, and status indicators. Each token has a light and a dark value; the **dark column applies when** dark mode is active via [`data-theme="dark"` or `data-theme="auto"`](/styling/theming/#dark-mode), and the **light column applies otherwise**.
